...
dashboardRouter.get('/dashboard', async (req, res) => {
  ...
  const layout = await withTenant(req.tenantId!, (db) => db.dashboardLayout.findUnique({ where: { userId } }));
...
dashboardRouter.put('/dashboard', async (req, res) => {
  ...
  const layout = await withTenant(req.tenantId!, (db) =>
    db.dashboardLayout.upsert({
      where: { userId },
      update: { charts },
