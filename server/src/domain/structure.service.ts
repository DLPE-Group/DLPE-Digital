...
export async function getTree(db: DbClient = prisma): Promise<TreeNode | null> {
...
export async function resolveSettingsFor(id: string, db: DbClient = prisma): Promise<Record<string, ResolvedSetting>> {
  const root = await getTree(db);
  if (!root) return {};
...
export async function addNode(
  ...
  const parent = await db.orgNode.findUnique({ where: { id: parentId } });
...
export async function deleteNode(id: string, db: DbClient = prisma): Promise<void> {
  const node = await db.orgNode.findUnique({ where: { id } });
  if (!node) throw new Error('Node not found');
  if (node.kind === 'GROUP') throw new Error('The group root cannot be deleted.');
  const children = await db.orgNode.count({ where: { parentId: id } });
  if (children > 0) throw new Error(`Node has ${children} child node(s) — remove them first.`);
  const entities = await db.entity.count({ where: { companyId: id } });
  if (entities > 0) throw new Error(`Node has ${entities} item(s) attached — remove them first.`);
  const [usersP, scopes] = await Promise.all([
    db.user.count({ where: { scopeNodeId: id } }),
    db.userScope.count({ where: { scopeNodeId: id } }),
  ]);
...
export async function updateNode(id: string, data: { name?: string; meta?: unknown; settings?: unknown; overrides?: unknown }, db: DbClient = prisma) {
  return db.orgNode.update({
    where: { id },
    data: {
      name: data.name,
      meta: (data.meta as object) ?? undefined,
