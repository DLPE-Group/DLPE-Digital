/* ============================================================
   AI Reporting area.
   Two surfaces:
     • Creation area — templates + natural-language composer that
       opens a short guided builder (period / scope / format).
     • View area (Library) — saved reports you can reopen.
   Numbers are always computed from the live DataSource (seed
   data) so charts never hallucinate; the AI writes the prose
   (live via window.claude.complete, with a scripted fallback).
   ============================================================ */
...
const ReportsView = () => {
  ...
  return (
    <div className="viewWrap">
      ...
      {tab === 'dashboard' ? (
        window.DashboardTab ? <DashboardTab /> : null
      ) : tab === 'create' ? (
