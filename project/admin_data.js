/* ============================================================
   ...
   Plain JS so it loads before the Babel scripts (like data.js).
   ============================================================ */
...
/* ---------- Group → Region/Country → Company tree ----------
   Each node: id, kind, name, code, optional settings + overrides.
   "own" settings = explicitly set at this node (override of parent).
*/
const GROUP_TREE = {
...
/* Data-sharing model — Group admin sets default sharing per data type */
const DATA_SHARING = [
...
/* ---------- Users (9, mapped to the dashboard sample data) ---------- */
const ADMIN_USERS = [
...
/* ---------- Data types and their fields, with per-role rules ----------
   Each field: id, label, group (category).
   ruleSets: { roleId: { visible, editable, masked, note } } — when a role
   isn't listed for a field, it falls back to the field's default rule.
*/
const FIELD_CATEGORIES = ['Identity', 'Commercial', 'Financial', 'Operational', 'Internal'];
...
const DEFAULT_RULE = { visible: true, editable: true, masked: false };

/* Per-role overrides. Keyed roleId -> dataTypeId -> fieldId -> rule.
   Built around the spec's demo: Sales rep cannot see workshop financials. */
const FIELD_RULES = {
  'sales-rep': {
    contract: {
...
Object.assign(window, {
  COUNTRY_DEFAULTS, GROUP_TREE, DATA_SHARING, ADMIN_USERS, SCOPE_TYPE_LABEL,
  ROLES, FIELD_CATEGORIES, DATA_TYPES, DEFAULT_RULE, FIELD_RULES, SAMPLE_RECORDS, RBAC_VERSIONS,
});
