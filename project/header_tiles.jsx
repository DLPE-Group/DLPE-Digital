...
const StatTile = ({ label, value, sub, accent, trend, active, onClick, bars }) => (
...
const StatRow = ({ active, setActive }) => (
...
Object.assign(window, { AppHeader, StatRow });

/* ============================================================
   ScorecardRow — replaces the abstract stat-tile row with one
   informational tile per track. Each tile shows count + health
   bar + the single most urgent item with its CTA.
   ============================================================ */
...
const TrackScorecard = ({ trackId, label, items, hero, isOpen, focused, onToggle, onAct }) => {
  const { t } = useT();
  ...
  // Pick the single most-urgent item — red first, then oldest amber
  const urgent = reds[0]
...
const ScorecardRow = ({ sales, ops, workshop, finance, openTracks, focused, only, onClearFilter, onToggle, onAct }) => {
  const { t } = useT();
...
Object.assign(window, { TrackScorecard, ScorecardRow });
